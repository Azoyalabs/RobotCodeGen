    ...
    fn to_trait_def(&self) -> String {
        ...
        return match self {
            MsgDescr::ExecuteMsg { ident: _, args: _ } => {
                format!("\tfn {camel_name}(&mut self, app: &mut App, contract: &Addr, caller: &Addr, {args_fn_string} funds: Vec<Coin>);")
            }
            MsgDescr::QueryMsg {
                ident: _,
                args: _,
                return_type,
            } => format!("\tfn {camel_name}(&self, app: &App, contract: &Addr, {args_fn_string}) -> {return_type};"),
        };
    }
